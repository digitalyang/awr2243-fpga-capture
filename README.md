...
常用 target 示例：`--target sanity`（默认）、`--target ddr`、`--target awr-bar0`（BAR0 译码 + AWR2243 CSR 的 cocotb 回归）。详细使用说明见 [BUILD_AND_VERIFY.md](BUILD_AND_VERIFY.md)。

### 仿真（Cocotb + Verilator）

RTL 仿真不依赖 Vivado 综合，使用 Verilator + cocotb：
...
| 全链路（extractor + packer + ringbuffer） | `python tb/cocotb/run_sim_pipeline.py` | 平台 pipeline 用例，不含单独 DDR 子系统 |

全链路仿真中 `ddr_ringbuffer_controller` 使用的 Xilinx XPM 双口 RAM 由 **`tb/models/xpm_memory_sdpram_stub.sv`** 提供纯 RTL 替代，因此无需 Vivado 即可完成 Verilator 编译与运行。
...
* BAR0 CSR 布局：**0x000–0x08C** 为 DDR ringbuffer CSR（RDL 导出）；**0x090 及以后** 为 AWR2243 控制 CSR，由 `rtl/axil/axil_bar0_decode.sv` 译码后接入 `awr2243_ctrl_top`（RDL 见 `rdl/awr2243_control.rdl`，配置说明见 [docs/awr2243_configuration.md](docs/awr2243_configuration.md)）
...
| DDR Ring CSR 定义 | `rdl/ddr_ringbuffer_controller.rdl` |
| DDR Ring CSR 生成头/示例 | `sw/include/generated/ddr_ringbuffer_controller_csr.h`、`sw/examples/ddr_ringbuffer_controller_csr_example.c` |
| AWR2243 控制 CSR 定义 | `rdl/awr2243_control.rdl` |
| AWR2243 控制 CSR 生成头 | `sw/include/generated/awr2243_control_csr.h` |
| BAR0 译码（ring 0x000–0x08C / AWR 0x090+） | `rtl/axil/axil_bar0_decode.sv`；AWR 配置说明 [docs/awr2243_configuration.md](docs/awr2243_configuration.md) |
| 核心 RTL 模块 | `rtl/core/csi_packet_extractor.sv`、`rtl/core/fixed_slot_packer.sv`、`rtl/core/ddr_ringbuffer_controller.sv`、`rtl/core/ddr_ringbuffer_controller_axil.sv`、`rtl/core/awr2243_ctrl_top.sv` |
| Verilator 仿真用 XPM 双口 RAM 替代 | `tb/models/xpm_memory_sdpram_stub.sv`（全链路仿真时由 `sim_platform_runner` 参与编译，无需 Vivado XPM） |
...
* sample 区固定按 1024×12B 展开，不足补 0；有效 payload 结束位置按「header + min(pkt_bytes - header - CQ, sample_area)」计算，最后一 beat 内超出部分填 0
* CQ 区固定 9B，按 raw12 格式承载原始 64bit CQ 数据
* 尾部 padding 至 64B 对齐
* 写入策略：beat 对齐且整 beat 在 sample 区内时走**快路径**（直写 `s_axis.tdata`）；否则走**慢路径**（与当前 beat 缓冲合并、滑动窗口处理 sample/CQ 边界）；慢路径下若「空缓冲 + 整 beat 在 sample 区」则**旁路**直写当前 beat，避免 tail 重排错位；发生 break（跨 beat 截断）时写入 **carry-over 生成的 beat** 而非 merged_beat，保证 64 字节等场景 beat1 正确
...
## 8.6 AWR2243 控制（RDL CSR + BAR0）

**RTL**：`rtl/core/awr2243_ctrl_top.sv`（对外 AXI4-Lite slave）；`rtl/axil/axil_bar0_decode.sv`（BAR0 地址译码）；寄存器由 **`rdl/awr2243_control.rdl`** 定义，PeakRDL 生成 `rtl/generated/awr2243_control_csr.sv` 与 **`sw/include/generated/awr2243_control_csr.h`**。

* BAR0 地址 **0x090+** 经译码后接入 AWR2243 控制块，偏移与 RDL 一致（VERSION、CONTROL、STATUS、ERR_CODE_*、STEP_ID_*、LAST_OPCODE_*、IRQ_COUNT、FAULT_COUNT、各 SCRIPT_ID、SCRATCH）
* CONTROL 为 singlepulse（start_init、start_sensor、stop_sensor、hard_reset、clear_error）；配置流程与寄存器表见 [docs/awr2243_configuration.md](docs/awr2243_configuration.md)
* 运行 BAR0 + AWR CSR 回归：`tools/build_and_verify.sh --target awr-bar0`

## 8.7 SPI Driver（AWR2243）

AWR2243 配置与脚本执行由 `awr2243_ctrl_top` 内 SPI/sequencer/script_ram 等完成；若需在系统侧扩展 profile/chirp/frame 写入，可接 script RAM 或现有 CSR。
...
**BAR0 布局**：**0x000–0x08C** 为 DDR ringbuffer CSR；**0x090 及以后** 为 AWR2243 控制 CSR（见 [docs/awr2243_configuration.md](docs/awr2243_configuration.md)）。以下为 ringbuffer 段。

Ringbuffer 寄存器由 **`rdl/ddr_ringbuffer_controller.rdl`** 定义，生成 RTL 与 C 头文件。Host 侧以 **`sw/include/generated/ddr_ringbuffer_controller_csr.h`** 和结构体 `ddr_ringbuffer_controller_csr_t`（总大小 0x90）为准。以下偏移与 RDL 一致。
...
当前仓库将两套 RDL 接成可重复生成的 CSR 流程。

## 17.1 DDR Ringbuffer CSR

**RDL**：`rdl/ddr_ringbuffer_controller.rdl`。产物包括：
...
安装本地依赖（首次）：`./tools/rdl/install_peakrdl_deps.sh`。生成：`./tools/rdl/generate_ddr_ringbuffer_controller_csr.sh`。依赖安装在 `tools/rdl/.deps/`。

## 17.2 AWR2243 控制 CSR

**RDL**：`rdl/awr2243_control.rdl`。产物包括：

* `rtl/generated/awr2243_control_csr_pkg.sv`、`rtl/generated/awr2243_control_csr.sv`
* `sw/include/generated/awr2243_control_csr.h`
* `tb/uvm/ral/awr2243_control_csr_ral_pkg.sv`（可选）

生成：`./tools/rdl/generate_awr2243_control_csr.sh`。BAR0 译码与配置说明见 [docs/awr2243_configuration.md](docs/awr2243_configuration.md)。

## 17.3 RTL 侧使用（DDR Ringbuffer）
