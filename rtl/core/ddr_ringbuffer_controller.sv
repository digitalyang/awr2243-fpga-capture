...
module ddr_ringbuffer_controller #(
    ...
    function automatic logic [KEEP_CNT_W-1:0] count_keep_bytes(
    ...
    function automatic logic [DESC_FIFO_AW-1:0] fifo_ptr_next(
        input logic [DESC_FIFO_AW-1:0] ptr
    );
    ...
    assign next_wr_ptr_c    = ring_addr_add(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, wr_ptr_r, cap_alloc_bytes_r);
    ...
    always_comb begin
        ...
        if (rd_consume_i && !rd_slot_req_i) begin
            if (!rd_busy_o && !desc_fifo_empty_c) begin
                ...
                rd_ptr_n                = ring_addr_add(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, rd_ptr_r, desc_head_alloc_bytes_c);
            ...
            RD_ST_IDLE: begin
                if (rd_slot_req_i && !rd_consume_i) begin
                    if (!cfg_enable_i || !cfg_ring_ok_c || desc_fifo_empty_c || rd_axis_valid_r) begin
                        ...
                        rd_first_seg_bytes_n      = (desc_head_bytes_c <= bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c)) ?
                                                     desc_head_bytes_c : bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c);
                        rd_second_seg_bytes_n     = desc_head_bytes_c -
                                                     ((desc_head_bytes_c <= bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c)) ?
                                                        desc_head_bytes_c : bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c));
                        rd_seg_idx_n              = '0;
                        rd_seg_addr_n             = desc_head_addr_c;
                        rd_seg_bytes_rem_n        = (desc_head_bytes_c <= bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c)) ?
                                                     desc_head_bytes_c : bytes_to_ring_end(cfg_ring_base_addr_i, cfg_ring_size_bytes_i, desc_head_addr_c);
    ...
    if (1) begin : g_static_checks
        ...
        `DDR_RING_STATIC_ASSERT(CFG_AXI_ADDR_W == AXI_ADDR_W, "CFG_AXI_ADDR_W must match AXI interface address width")
        `DDR_RING_STATIC_ASSERT($bits(s_axis_slot.tdata) == AXIS_DATA_W, "s_axis_slot.tdata width must match AXIS_DATA_W")
        `DDR_RING_STATIC_ASSERT($bits(s_axis_slot.tkeep) == AXIS_KEEP_W, "s_axis_slot.tkeep width must match AXIS_KEEP_W")
        `DDR_RING_STATIC_ASSERT($bits(s_axis_slot.tuser) == AXIS_USER_W, "s_axis_slot.tuser width must match AXIS_USER_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axi.awaddr) == AXI_ADDR_W, "m_axi.awaddr width must match AXI_ADDR_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axi.wdata) == AXI_DATA_W, "m_axi.wdata width must match AXI_DATA_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axi.wstrb) == AXI_STRB_W, "m_axi.wstrb width must match AXI_STRB_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axis_rd.tdata) == RD_AXIS_DATA_W, "m_axis_rd.tdata width must match RD_AXIS_DATA_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axis_rd.tkeep) == RD_AXIS_KEEP_W, "m_axis_rd.tkeep width must match RD_AXIS_KEEP_W")
        `DDR_RING_STATIC_ASSERT($bits(m_axis_rd.tuser) == RD_AXIS_USER_W, "m_axis_rd.tuser width must match RD_AXIS_USER_W")
        `DDR_RING_STATIC_ASSERT(AXIS_DATA_W == AXI_DATA_W, "This revision requires AXIS and AXI write widths to match")
        `DDR_RING_STATIC_ASSERT(RD_AXIS_DATA_W == AXI_DATA_W, "Read AXIS width must match AXI data width")
        `DDR_RING_STATIC_ASSERT(AXIS_KEEP_W == AXI_STRB_W, "AXIS keep width must match AXI strobe width")
        `DDR_RING_STATIC_ASSERT(RD_AXIS_KEEP_W == AXI_STRB_W, "Read AXIS keep width must match AXI strobe width")
        ...
        `DDR_RING_STATIC_ASSERT(SLOT_BUFFER_BYTES >= AXI_BEAT_BYTES, "SLOT_BUFFER_BYTES must be at least one AXI beat")
        ...
        `DDR_RING_STATIC_ASSERT(ADDR_ALIGN_BYTES >= AXI_BEAT_BYTES, "ADDR_ALIGN_BYTES must be >= AXI beat bytes")
        `DDR_RING_STATIC_ASSERT((ADDR_ALIGN_BYTES % AXI_BEAT_BYTES) == 0, "ADDR_ALIGN_BYTES must be AXI beat aligned")
        `DDR_RING_STATIC_ASSERT((RD_TUSER_SEQ_LSB + SLOT_SEQ_W) <= RD_AXIS_USER_W, "RD TUSER seq field out of range")
        `DDR_RING_STATIC_ASSERT((RD_TUSER_BYTES_LSB + SLOT_BYTES_W) <= RD_AXIS_USER_W, "RD TUSER bytes field out of range")
        `DDR_RING_STATIC_ASSERT(RD_TUSER_SLOT_FIRST_BIT < RD_AXIS_USER_W, "RD_TUSER_SLOT_FIRST_BIT out of range")
        `DDR_RING_STATIC_ASSERT(RD_TUSER_SLOT_LAST_BIT < RD_AXIS_USER_W, "RD_TUSER_SLOT_LAST_BIT out of range")
        `DDR_RING_STATIC_ASSERT(RD_TUSER_VALID_GOOD_BIT < RD_AXIS_USER_W, "RD_TUSER_VALID_GOOD_BIT out of range")
        `DDR_RING_STATIC_ASSERT(RD_TUSER_OVERFLOW_ERR_BIT < RD_AXIS_USER_W, "RD_TUSER_OVERFLOW_ERR_BIT out of range")
    end
