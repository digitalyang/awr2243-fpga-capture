...
module csi_packet_extractor #(
    ...
    if (1) begin : g_static_checks
        ...
        `CSI_STATIC_ASSERT($bits(m_axis.tdata) == AXIS_DATA_W, "input/output AXIS data widths must match")
        `CSI_STATIC_ASSERT($bits(m_axis.tkeep) == AXIS_KEEP_W, "input/output AXIS keep widths must match")
        `CSI_STATIC_ASSERT($bits(m_axis.tuser) == AXIS_USER_W, "input/output AXIS user widths must match")
        ...
        `CSI_STATIC_ASSERT((TUSER_PAYLOAD_LEN_LSB + PAYLOAD_LEN_W) <= AXIS_USER_W, "TUSER_PAYLOAD_LEN field out of range")
    end
