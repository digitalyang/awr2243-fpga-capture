...
module ddr_ringbuffer_controller_axil #(
    ...
    `DDR_RING_STATIC_ASSERT(CFG_RING_BYTES_W <= 32, "CSR RDL exposes ring size and occupancy counters as 32-bit values")
    ...
    `DDR_RING_STATIC_ASSERT(COUNTER_W <= 32, "CSR RDL exposes wrap/overflow/drop counters as 32-bit values")
    `DDR_RING_STATIC_ASSERT($bits(s_axil.wdata) == CSR_DATA_W, "AXI-Lite data width must match generated CSR block")
    `DDR_RING_STATIC_ASSERT($bits(s_axil.awaddr) >= CSR_ADDR_W, "AXI-Lite address width must cover the generated CSR space")
    ...
    assign cfg_ring_size_bytes_c  = csr_hwif_out.RING_SIZE_BYTES.ring_size_bytes.value[CFG_RING_BYTES_W-1:0];
    ...
    always_comb begin
        ...
        csr_hwif_in.RING_SIZE_BYTES.ring_size_bytes.next     = csr_hwif_out.RING_SIZE_BYTES.ring_size_bytes.value;
