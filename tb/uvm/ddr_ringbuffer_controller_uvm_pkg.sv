...
package ddr_ringbuffer_controller_uvm_pkg;
    ...
    class ddr_ring_slot_item extends uvm_sequence_item;
        ...
        constraint c_payload_size {
            payload.size() inside {[1 : (TB_AXI_BEAT_BYTES * 8)]};
        }
    ...
    class ddr_ring_axi_mem_model extends uvm_component;
        ...
        function void write_mem_word(
            input longint unsigned addr,
            input logic [TB_AXI_DATA_W-1:0] data,
    ...
    class ddr_ring_scoreboard extends uvm_component;
        ...
        function void compare_slot_meta(
            input string              prefix,
            input ddr_ring_slot_item  exp_item,
            input bit [TB_SLOT_SEQ_W-1:0] act_seq,
            input int unsigned        act_bytes,
            input bit                 act_valid,
            ...
            if (exp_item.slot_bytes() != act_bytes) begin
                push_error($sformatf("%s bytes mismatch exp=%0d act=%0d", prefix, exp_item.slot_bytes(), act_bytes));
            end
            if (exp_item.slot_valid_good != act_valid) begin
                push_error($sformatf("%s valid_good mismatch exp=%0b act=%0b", prefix, exp_item.slot_valid_good, act_valid));
            end
            if (exp_item.slot_overflow_err != act_overflow) begin
                push_error($sformatf("%s overflow mismatch exp=%0b act=%0b", prefix, exp_item.slot_overflow_err, act_overflow));
            end
        endfunction

        function void compare_payload(
            input string            prefix,
            input byte unsigned     exp_payload[$],
            input byte unsigned     act_payload[$]
        );
            if (exp_payload.size() != act_payload.size()) begin
                push_error($sformatf("%s payload size mismatch exp=%0d act=%0d",
                                     prefix, exp_payload.size(), act_payload.size()));
                return;
            end

            foreach (exp_payload[idx]) begin
                if (exp_payload[idx] != act_payload[idx]) begin
                    push_error($sformatf("%s payload mismatch at byte %0d exp=0x%0h act=0x%0h",
                                         prefix, idx, exp_payload[idx], act_payload[idx]));
                    break;
                end
            end
        endfunction
        ...
        function void write_commit(ddr_ring_commit_item item);
            ...
            begin
                if (exp_commit_q.size() == 0) begin
                    push_error($sformatf("Unexpected commit, seq=%0d addr=0x%0h", item.slot_seq, item.slot_addr));
                    return;
                end

                exp_item = exp_commit_q.pop_front();
                compare_slot_meta("commit", exp_item, item.slot_seq, item.slot_bytes, item.slot_valid_good, item.slot_overflow_err);

                if (mem_model == null) begin
                    push_error("mem_model is null in scoreboard");
                end else begin
                    mem_model.read_ring_bytes(
                        cfg.cfg_ring_base_addr,
                        cfg.cfg_ring_size_bytes,
                        item.slot_addr,
                        exp_item.slot_bytes(),
                        mem_bytes
                    );
        ...
        function void write_read(ddr_ring_slot_item item);
            ...
            begin
                if (exp_read_q.size() == 0) begin
                    push_error($sformatf("Unexpected read slot, seq=%0d bytes=%0d",
                                         item.slot_seq, item.slot_bytes()));
                    return;
                end
        ...
        function void report_phase(uvm_phase phase);
            ...
            if (!is_idle()) begin
                push_error($sformatf("Scoreboard still busy: exp_commit_q=%0d exp_read_q=%0d",
                                     exp_commit_q.size(),
                                     exp_read_q.size()));
            end
    ...
    class ddr_ring_base_sequence extends uvm_sequence #(ddr_ring_slot_item);
        ...
        virtual task send_slot(
            input int unsigned             byte_count,
            input bit [TB_SLOT_SEQ_W-1:0]  slot_seq,
            input byte unsigned            seed,
            input bit                      slot_valid_good = 1'b1,
    ...
    class ddr_ring_base_test extends uvm_test;
        ...
        function void build_phase(uvm_phase phase);
            ...
            if (!uvm_config_db#(virtual axis_stream_if #(TB_AXI_DATA_W, TB_RD_AXIS_USER_W))::get(this, "", "rd_vif", m_cfg.rd_vif)) begin
                `uvm_fatal(get_type_name(), "rd_vif not found")
            end
            if (!uvm_config_db#(virtual axi4_mm_if #(TB_AXI_ADDR_W, TB_AXI_DATA_W))::get(this, "", "axi_vif", m_cfg.axi_vif)) begin
