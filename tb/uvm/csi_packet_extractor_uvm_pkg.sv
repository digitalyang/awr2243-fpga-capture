...
// UVM environment, sequences, monitors, and scoreboard for extractor verification.
package csi_packet_extractor_uvm_pkg;
  ...
  function automatic logic [TB_AXIS_USER_W-1:0] build_tuser(
      input logic [TB_VC_W-1:0] vc, input logic [TB_DT_W-1:0] dt,
      input logic [TB_PAYLOAD_LEN_W-1:0] payload_len, input logic crc_err, input logic ecc_err,
      input logic trunc_err, input logic pkt_start);
  ...
  class csi_scoreboard extends uvm_component;
    ...
    function void write_exp(csi_axis_packet_item item);
      ...
      begin
        if (packet_matches(item)) begin
          ...
          `uvm_info(get_type_name(), $sformatf("Expect accept: %s", item.convert2string()),
                    UVM_MEDIUM)
        end else begin
          `uvm_info(get_type_name(), $sformatf("Expect drop: %s", item.convert2string()),
                    UVM_MEDIUM)
        end
      end
    endfunction
    ...
    function void write_act(csi_axis_packet_item item);
      ...
      begin
        ...
        if (item.payload.size() != exp_item.payload.size()) begin
          push_error($sformatf(
                     "Payload byte count mismatch, exp=%0d act=%0d",
                     exp_item.payload.size(),
                     item.payload.size()
                     ));
        end
        ...
        foreach (exp_item.payload[idx]) begin
          if ((idx >= item.payload.size()) || (item.payload[idx] != exp_item.payload[idx])) begin
            push_error($sformatf(
                       "Payload mismatch at byte %0d, exp=0x%0h", idx, exp_item.payload[idx]));
            break;
          end
        end
      end
        ...
        case (item.kind)
          META_KIND_START: begin
            ...
            if (item.pkt_seq != exp_seq) begin
              push_error($sformatf("pkt_start seq mismatch, exp=%0d act=%0d", exp_seq, item.pkt_seq
                         ));
            end
          end
          ...
          META_KIND_DONE: begin
            ...
            if (item.pkt_seq != exp_item.pkt_seq) begin
              push_error($sformatf(
                         "pkt_done seq mismatch, exp=%0d act=%0d", exp_item.pkt_seq, item.pkt_seq));
            end
            if (item.pkt_bytes != exp_item.pkt_bytes) begin
              push_error(
                  $sformatf(
                  "pkt_done bytes mismatch, exp=%0d act=%0d", exp_item.pkt_bytes, item.pkt_bytes));
            end
            if (item.pkt_crc_err != exp_item.pkt_crc_err) begin
              push_error($sformatf(
                         "pkt_done crc_err mismatch, exp=%0b act=%0b",
                         exp_item.pkt_crc_err,
                         item.pkt_crc_err
                         ));
            end
            if (item.pkt_ecc_err != exp_item.pkt_ecc_err) begin
              push_error($sformatf(
                         "pkt_done ecc_err mismatch, exp=%0b act=%0b",
                         exp_item.pkt_ecc_err,
                         item.pkt_ecc_err
                         ));
            end
            if (item.pkt_trunc_err != exp_item.pkt_trunc_err) begin
              push_error($sformatf(
                         "pkt_done trunc_err mismatch, exp=%0b act=%0b",
                         exp_item.pkt_trunc_err,
                         item.pkt_trunc_err
                         ));
            end
            if (item.pkt_valid_good != exp_item.pkt_valid_good) begin
              push_error($sformatf(
                         "pkt_done valid_good mismatch, exp=%0b act=%0b",
                         exp_item.pkt_valid_good,
                         item.pkt_valid_good
                         ));
            end
          end
    ...
    function bit is_idle();
      return (exp_start_q.size() == 0) && (exp_data_q.size() == 0) && (exp_meta_q.size() == 0);
    endfunction
    ...
    function void report_phase(uvm_phase phase);
      ...
      if (!is_idle()) begin
        push_error($sformatf(
                   "Scoreboard still has pending expectations: start=%0d data=%0d meta=%0d",
                   exp_start_q.size(),
                   exp_data_q.size(),
                   exp_meta_q.size()
                   ));
  ...
  class csi_base_sequence extends uvm_sequence #(csi_axis_packet_item);
    ...
    virtual task send_pkt(input bit [TB_VC_W-1:0] vc, input bit [TB_DT_W-1:0] dt,
                          input int unsigned actual_nbytes, input int unsigned declared_nbytes,
                          input bit crc_err_first = 1'b0, input bit ecc_err_first = 1'b0,
                          input bit trunc_err_first = 1'b0, input bit crc_err_last = 1'b0,
                          input bit ecc_err_last = 1'b0, input bit trunc_err_last = 1'b0);
